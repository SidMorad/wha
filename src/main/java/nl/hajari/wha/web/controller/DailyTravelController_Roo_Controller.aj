...
privileged aspect DailyTravelController_Roo_Controller {
    ...
    public String DailyTravelController.findDailyTravelsByDayDateAndTimesheetForm(ModelMap modelMap) {
        modelMap.addAttribute("timesheets", Timesheet.findAllTimesheets());
        return "dailytravel/findDailyTravelsByDayDateAndTimesheet";
    }
    
    @RequestMapping(value = "find/ByDayDateAndTimesheet", method = RequestMethod.GET)
    public String DailyTravelController.findDailyTravelsByDayDateAndTimesheet(@RequestParam("dayDate") Date dayDate, @RequestParam("timesheet") Timesheet timesheet, ModelMap modelMap) {
        if (dayDate == null) throw new IllegalArgumentException("A DayDate is required.");
        if (timesheet == null) throw new IllegalArgumentException("A Timesheet is required.");
        modelMap.addAttribute("dailytravels", DailyTravel.findDailyTravelsByDayDateAndTimesheet(dayDate, timesheet).getResultList());
        return "dailytravel/list";
    }
